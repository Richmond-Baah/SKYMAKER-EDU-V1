    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button variant="ghost" size="sm" className="gap-2 text-white/60 hover:text-white px-3 h-8 text-[11px] font-bold border border-white/5 bg-white/5 hover:bg-white/10 transition-all">
                    <Book className="w-3.5 h-3.5" />
                    PILOT'S MANUAL
                </Button>
            </DialogTrigger>
            <DialogContent className="max-w-4xl h-[85vh] bg-[#0a0a0c] border border-white/10 text-white p-0 overflow-hidden flex flex-col shadow-2xl antialiased">
                <DialogHeader className="p-8 border-b border-white/5 bg-gradient-to-br from-blue-600/10 to-transparent shrink-0">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            ...
                            <DialogDescription className="text-white/40 text-[10px] font-bold uppercase tracking-widest mt-0.5">
                                ...
                                {categories.map((cat, i) => (
                                    <div key={i} className="space-y-4">
                                        <div className="flex items-center gap-2 pb-2 border-b border-white/5">
                                            {cat.icon}
                                            <h3 className="font-bold text-xs tracking-widest uppercase text-white/80">{cat.title}</h3>
                                        </div>
                                        <div className="space-y-4">
                                            {cat.commands.map((cmd, j) => (
                                                <div key={j} className="bg-white/[0.02] p-5 rounded-xl border border-white/5 space-y-3 hover:border-white/10 transition-colors group">
                                                    <div className="flex items-center justify-between">
                                                        <code className="text-blue-400 font-bold text-sm font-mono tracking-tighter bg-blue-400/5 px-2 py-0.5 rounded">
                                                            {cmd.name}
                                                        </code>
                                                        <ChevronRight className="w-4 h-4 text-white/5 group-hover:text-blue-400 group-hover:translate-x-1 transition-all" />
                                                    </div>
                                                    ...
                                                    <div className="bg-black/40 rounded-lg p-3 border border-white/5 flex items-center justify-between">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <code className="text-[10px] text-emerald-400/80 font-mono italic">
                                                                {cmd.example}
                                                            </code>
                                                        </div>
                                                        <span className="text-[9px] text-white/10 font-bold uppercase tracking-tighter">Usage Code</span>
                                                    </div>
                ...
                <div className="p-4 bg-black/40 border-t border-white/5 flex items-center justify-center gap-4 shrink-0">
                    <span className="text-[10px] text-white/20 font-bold tracking-widest uppercase">Safe Flight Protocol Active</span>
                    <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
                    <span className="text-[10px] text-white/20 font-bold tracking-widest uppercase">Encryption Secure</span>
                </div>
            </DialogContent>
        </Dialog>
    )
